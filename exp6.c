...
float FCFS(P p[], int n){
    int time=0;
    for(int i=0;i<n;i++){
        if(time<p[i].at) time=p[i].at;//arrival check
        time+=p[i].bt;//executes
        p[i].ct=time;
...
float SRTF(P p[], int n){
    ...
    while(done<n){
        ...
        for(int i=0;i<n;i++)
            if(p[i].at<=time && p[i].rt>0)//checking process actually arrived & work left to do
                if(idx==-1 || p[i].rt<p[idx].rt)
                    idx=i;

        if(idx==-1){ //no process yet arrived
            time++;
        }
        else{
            p[idx].rt--;// to make process preemptive
            time++;

            if(p[idx].rt==0){//after all done
                done++;
                p[idx].ct=time;
    ...
    while(done<n){
        ...
        for(int i=0;i<n;i++)
            if(p[i].at<=time && !vis[i])
                if(idx==-1 || p[i].pr>p[idx].pr) //priority checking
                    idx=i;
...
int main(){
	...
	if(b < min){ min = b; strcpy(algo,"SRTF"); }
